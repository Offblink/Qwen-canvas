...
- **生成**：提示词 / 尺寸 / 步数 / 张数 / 种子（都是自绘控件）/ 参考图。点生成**不动画布**；提示词在**提交那一刻**
  就交出去并清空（失败会还原），参考图与各参数留着。**等图的时候面板不锁**：页签照切、参数照调（下一轮用）、
  历史照翻照拖照换图；只有两页底部的动作按钮灰着（免得重复提交），画布上也不接新框（那张图马上要被换了）；
  跑完如果你已经点了别的图，结果只进历史，**不把画布抢回来**。
