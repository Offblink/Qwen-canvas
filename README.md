...
左边那块画布显示谁，按页签分：**「生成」页只显示本次会话生成出来的结果**（没生成过就是一句引导文案），
**「修改 / 历史」页显示当前底图**。理由：生成请求根本不读画布上那张图（它只吃提示词和参考图区），
在生成页摆一张历史图只会让人以为"在用它生成 / 在改它"。
...
**参考图（生成页）是"编辑"语义**：图喂给 `TextEncodeQwenImage21` 的 `images.image_i`，缩略图上的 1/2/3 徽标对应提示词里写的编号；
提示词要写**「把 1 里的…换成…」**这种改动指令 —— **写纯描述会原样返回**（实测同一张图、同 seed：指令句只在指定区域变了
29.1/255，描述句那块只差 2.1，等于什么都没改；数字见 `CANVAS_NOTES.md`）。多图「把 A 的元素放进 B」很弱。

| 操作 | 效果 |
|---|---|
| 点 **生成** | 画布**不动**（生成期间上一张照旧在那儿、照旧能拖框，出图时才被新图替换）；**出图后清空提示词**，参考图与尺寸/步数/张数/种子留着；**失败不清**，提示词留着改完重试 |
