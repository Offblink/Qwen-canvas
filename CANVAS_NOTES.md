...
**点「生成」不动画布**（生成期间那张图照旧显示；出图时才被新图替换。生成期间**不接新框**，见下节）。出图成功后**清空生成页的提示词**
（`$('prompt').value=''`，状态栏那行也会写"提示词已清空"）；**参考图、尺寸、步数、张数、种子都留着** ——
参考图重传成本高，尺寸/步数是设置，种子是"同一张重跑/对比"要用的。**失败时不清**，提示词留着好改完重试。
...
### 出图期间锁什么（2026-09-22 改）

`busyOn(t)` 只把**两页底部的动作按钮**置灰：`生成` / `应用修改` / `整图按建议重画` / `清除全部框`
（免得重复提交 —— 一次请求就吃满 12 GB 卡）。**其余一律照常能用**：页签随便切、尺寸/步数/张数/种子随便调、
参考图随便加、提示词优化开关随便拨、「改前 / 改后」照看、`ComfyUI 原生界面` 照开、历史页照翻照删。
理由：这些值**都是提交那一刻才读的**（`btnGen` 里先 `segSize.get()` 再发请求），改了只影响下一轮，
不会动到正在跑的那次。实测 512²/6 步出图中：4 个底部按钮全灰、参数控件 14 个 0 个被锁、三个页签都能切且切完就渲染，
状态栏进度环照转（截图里就是"历史页翻着、图在下面出"）。
另外 `paintOpt()` / `updateCmpBtn()` 里不再掺 `busy`（优化开关看"有没有配 key"，对比按钮看"有没有改前改后这一对"）。

仍然按下的四处，**都带状态栏提示，不静默失败**（前三条都是因为"跑完会清空/重画"）：

| 出图中做 | 行为 |
|---|---|
| 在画布上拖新框 | 不接 → 「出图中 —— 这张跑完再拖框」（跑完 `regions` 会被清空，画了白画） |
| 点历史图换底图 | 不接 → 「跑完再点图换底图」（跑完 `show(j.image)` 把画布换成新图，你选的会被顶掉） |
| 拖动历史格子换顺序 | 不接 → 「跑完再换顺序」（`pushHist` → `renderHist` 会重画整个网格，拖一半会被抽走） |
| 在区域卡片里改建议 | **允许**，只提示一次「跑完会清空所有框和建议，先别在这儿改」 |
| 删图（×）/ 开 ComfyUI 原生界面 / 切页签 / 调参数 | 允许 |
