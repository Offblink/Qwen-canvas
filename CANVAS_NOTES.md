...
**另外只挡一处：画布上拖新框**（`wrap` mousedown 里的 `busy` 判断）—— 那张图马上就要被出图结果替换
（文生图/整图重画还可能换尺寸），这时候画的框对不上新图；挡的同时给状态栏一句「出图中 —— 这张跑完再拖框」。
**其余全都不锁**：页签随便切、尺寸/步数/张数/种子随便调、参考图随便加、优化开关随便拨、改前/改后照看、
历史照翻**照点换底图照拖排序照删**、已有的框照点照删、区域卡片里照写建议。理由：这些都是"**提交那一刻才读**"的状态
（`btnGen` 里先 `segSize.get()` 再发请求），改了只影响下一轮；上一批框在提交时就已经交给服务端了。

配套的两条设计（否则"放开"就等于"静默丢你的东西"）：

1. **提交即清框**（`takeRegions()`）。点击那一刻就把框和建议交出去、画布立刻空出来（这批已经交给服务端了，
   跑完也不会再动它）。**请求失败会还原**（`restoreRegions()`，且只在你还什么都没画的时候还原）。
...
`busy` 这个变量留着 —— 只有画布拖框那一处读它。

实测（1024²/25 步真出图，出图中逐项做了一遍）：
- 提示词在 `status=生成中` 时已是空串（提交即清）；4 个底部按钮全 disabled、三个页签都可用。
- 点历史第 4 张 → 画布换成它、提示「已把 … 设为底图」。
- **在画布上拖框 → 一个新框都没出现**（`#regions .reg` = 0，`#draft` 没显示），状态栏给「出图中 —— 这张跑完再拖框」。
- 写建议、去生成页写下轮提示词、拖历史格子换顺序（DOM 与服务端都变、补位动画照跑）全都成功。
- 跑完（18.2 s）：新图落在历史第 1 格、**画布仍停在出图前你点的那张**（不抢）、写下的下轮提示词**还在**、
  底部按钮自动恢复、进度环熄灭。
